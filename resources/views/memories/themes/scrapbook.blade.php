  ...
  <div class="pt-24 bg-[#d7c7b4]">
    ...
    <section class="relative max-w-7xl mx-auto px-8 py-14 md:py-18 overflow-hidden">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        <div class="relative">
          {{-- Tape strips --}}
          <div class="absolute -top-6 left-12 w-32 h-6 bg-rose-200/70 rotate-[-18deg] rounded-md blur-[0.2px]"></div>
          <div class="absolute -right-10 top-6 w-28 h-6 bg-rose-200/60 rotate-[12deg] rounded-md"></div>
        ...
        <div class="relative">
          <div class="inline-flex items-center gap-3 px-4 py-2 rounded-full border border-stone-300 bg-white/60 backdrop-blur">
            ...
            <span class="text-xs tracking-widest uppercase text-stone-600">Scrapbook Theme</span>
          </div>
          ...
          @if(!empty($memory->opening_quote))
            <div class="mt-6 bg-white/65 border border-stone-200 rounded-3xl p-7 relative">
              {{-- Sticky paper effect --}}
              <div class="absolute -top-3 left-8 w-16 h-6 bg-rose-200/60 rotate-[-10deg] rounded-md"></div>
    ...
    <section class="max-w-7xl mx-auto px-8 pb-14">
      @include('partials.lightbox', ['id'=>'scrapbook-lb'])
      @include('partials.music-player', ['musicUrl'=>$memory->music_url, 'accentColor'=>$memory->getAccentColor(), 'playerId'=>'scrapbook-music'])
        ...
        @foreach($memory->sections as $i => $s)
          @if($s->type === 'story')
            <div class="relative bg-white/65 border border-stone-200 rounded-3xl p-8 shadow-sm">
              <div class="absolute -top-4 left-8 w-24 h-8 bg-rose-200/70 rotate-[-12deg] rounded-md"></div>
          ...
          @elseif($s->type === 'timeline')
            <div class="relative bg-white/55 border border-stone-200 rounded-3xl p-8 overflow-hidden">
              <div class="absolute -left-6 top-10 w-12 h-12 rounded-full bg-rose-200/40"></div>
              <div class="flex gap-8">
                <div class="min-w-[7rem] text-right">
                  @if($s->time_label)
                    <div class="text-xs tracking-widest uppercase text-stone-600">{{ $s->time_label }}</div>
                  @endif
                  <div class="mt-4 w-10 h-10 rounded-full border border-stone-200 bg-white flex items-center justify-center text-rose-400">♡</div>
                </div>
                <div class="flex-1">
                  @if($s->heading)
          ...
          @elseif($s->type === 'quote')
            <div class="bg-white/65 border border-stone-200 rounded-3xl p-10 text-center relative">
              <div class="absolute -top-3 right-10 w-28 h-7 bg-rose-200/70 rotate-[8deg] rounded-md"></div>
              @if($s->heading)
