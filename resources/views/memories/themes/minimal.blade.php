...
@section('content')
  <div class="pt-24 bg-stone-50 text-stone-800">
        ...
        <div class="relative grid grid-cols-5 gap-0 items-stretch">
          ...
          <div class="col-span-2 p-10 flex flex-col justify-center">
            <div class="inline-flex items-center gap-3 px-4 py-2 rounded-full border border-stone-200 bg-white">
              ...
              <span class="text-xs tracking-widest uppercase text-stone-500">{{ $memory->category?->name ?? 'Memories' }}</span>
            </div>

            <h1 class="mt-6 font-display text-4xl leading-tight">{{ $memory->title }}</h1>

            @if(!empty($memory->opening_quote))
              <p class="mt-4 text-stone-700 italic text-lg leading-relaxed">
    ...
    <section class="max-w-7xl mx-auto px-8 pb-12">
      @include('partials.lightbox', ['id'=>'minimal-lb'])
      @include('partials.music-player', ['musicUrl'=>$memory->music_url, 'accentColor'=>$memory->getAccentColor(), 'playerId'=>'minimal-music'])

      <h2 class="font-script text-3xl text-stone-900">Gallery</h2>
      <p class="mt-2 text-xs tracking-widest uppercase text-stone-500">Clean, calm, and click to open</p>
        ...
        @foreach($memory->galleryImages as $img)
          <button type="button"
            ...
            @if($img->caption || $img->handwritten_caption)
              <div class="px-3 py-2 text-xs text-stone-600 line-clamp-1">
    ...
    <section class="max-w-7xl mx-auto px-8 pb-20">
      <div class="space-y-10">
        @foreach($memory->sections as $i => $s)
              ...
              <div class="{{ $s->image_right ? 'md:flex-row-reverse' : 'md:flex-row' }} flex flex-col md:flex gap-8 items-center">
                @if($s->image)
                  <div class="w-full md:w-1/2">
                    ...
                    @if($s->image_tag)
                      <div class="mt-3 text-center text-xs tracking-widest uppercase text-stone-500">{{ $s->image_tag }}</div>
                    @endif
                  </div>
                @endif
                <div class="flex-1">
                  @if($s->label)
                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-stone-200 bg-stone-50">
                      <span class="inline-block w-2 h-2 rounded-full" style="background: {{ $memory->getAccentColor() }}"></span>
                      <span class="text-xs tracking-widest uppercase text-stone-500">{{ $s->label }}</span>
                    </div>
                  @endif
                  @if($s->heading)
                    <h3 class="mt-4 font-display text-3xl text-stone-900 leading-tight">{!! $s->heading !!}</h3>
                  @endif
                  @if($s->content)
          ...
          @elseif($s->type === 'quote')
            <div class="rounded-3xl border border-stone-200 bg-stone-50 p-10 text-center">
              <div class="mx-auto max-w-3xl">
                @if($s->heading)
                  <h3 class="font-display text-3xl text-stone-900">{!! $s->heading !!}</h3>
                @endif
                @if($s->content)
                  <blockquote class="mt-6 text-stone-700 text-xl md:text-2xl leading-relaxed italic">
                    {!! $s->content !!}
                  </blockquote>
                @endif
                @if($s->quote_author)
