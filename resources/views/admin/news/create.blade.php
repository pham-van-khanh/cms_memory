...
@section('content')
  <div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
      <h2 class="font-display text-2xl text-stone-900">Create News</h2>
      <p class="text-sm text-stone-600 mt-1">Add a short announcement and decide whether it’s published.</p>

      <form class="mt-6 space-y-5" method="POST" action="{{ route('admin.news.store') }}">
        @csrf
        ...
        <div>
          ...
          <textarea name="content" rows="9" class="mt-2 w-full rounded-xl border border-stone-200 px-4 py-2 text-sm">{{ old('content') }}</textarea>
        </div>

        <div class="flex items-center gap-3">
          <input type="hidden" name="published" value="0">
          <input type="checkbox" name="published" value="1" id="published" class="h-4 w-4" {{ old('published') ? 'checked' : '' }}>
