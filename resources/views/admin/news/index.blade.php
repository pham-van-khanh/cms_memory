...
@section('content')
  <div class="space-y-5">
    <div class="flex flex-wrap items-end justify-between gap-4">
      <div>
        <h2 class="font-display text-2xl text-stone-900">News</h2>
        <p class="text-sm text-stone-600 mt-1">Short announcements with a clean publish workflow.</p>
      </div>
          ...
          <tbody>
            @foreach($posts as $p)
              <tr class="border-t border-stone-100">
                ...
                <td class="py-3 px-2">
                  <form method="POST" action="{{ route('admin.news.publish',$p) }}">
                    @csrf
                    @method('PATCH')
                ...
                <td class="py-3 px-2 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <a href="{{ route('admin.news.edit',$p) }}" class="text-xs px-3 py-1 rounded-xl border border-stone-200 hover:bg-stone-50 transition text-stone-700">
                      Edit
                    </a>
                    <form method="POST" action="{{ route('admin.news.destroy',$p) }}" onsubmit="return confirm('Delete this post?')">
                      @csrf
                      @method('DELETE')
